...
\begin{theorem}
  Suponiendo \(\phi \in L^1\) y \(\int \phi(x) \dd x = a\).
  Para cada \(\varepsilon > 0\), definimos \(\phi_{\varepsilon}\) mediante \(\phi_{\varepsilon}(x) = \varepsilon^{- n} \phi(\varepsilon^{- 1} x)\).
  Si \(f \in L^p\), \(1 \leq p < \varepsilon\), entonces \(f * \phi_{\varepsilon} \rightarrow a f\) en norma \(L^p\) cuando \(\varepsilon \rightarrow 0\).
  Si \(f \in L^{\infty}\) y \(f\) es uniformemente continua en un conjunto \(V\), entonces \(f * \phi_{\varepsilon} \rightarrow a f\) uniformemente en \(V\) cuando \(\varepsilon \rightarrow 0\).
\end{theorem}
\begin{proof}
  Por el cambio de variable \(x \rightarrow \varepsilon x\) vemos que \(\int \phi_{\varepsilon}(x) \dd x = a\) para todo \(\varepsilon > 0\).
  Luego,
  \begin{align}
    f * \phi_{\varepsilon} (x) - a f(x)
    &=
    \int (f(x - y) - f(x)) \phi_{\varepsilon}(y) \dd y
    \\
    &=
    \int (f(x - \varepsilon y) - f(x)) \phi(y) \dd y
  \end{align}
  Si \(f \in L^p\) y \(p < \infty\), aplicamos la desigualdad triangular para integrales (llamada desigualdad de Minkowski) para obtener
  \begin{align}
    \norm{f * \phi_{\varepsilon} - a f}_p
    \leq
    \int \norm{f_{- \varepsilon y} - f}_p \abs{\phi(y)} \dd y
  \end{align}
